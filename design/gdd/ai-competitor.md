...
|------|-------------|-------------|---------|-----------|-------|-------|
| 브론즈 | 1,000,000 | 3,000,000 | 25.0% | 55% | -60% | 600% |
| 실버 | 3,000,000 | 10,000,000 | 40.0% | 50% | -60% | 650% |
| 골드 | 10,000,000 | 30,000,000 | 60.0% | 45% | -60% | 550% |
| 플래티넘 | 30,000,000 | 100,000,000 | 80.0% | 40% | -60% | 500% |
| 에메랄드 | 100,000,000 | 300,000,000 | 105.0% | 35% | -60% | 450% |
| 다이아 | 300,000,000 | 1,000,000,000 | 130.0% | 30% | -60% | 420% |
| 마스터 | 1,000,000,000 | 3,000,000,000 | 155.0% | 25% | -60% | 380% |
| 그랜드마스터 | 3,000,000,000 | 10,000,000,000 | 180.0% | 22% | -60% | 360% |
| 챌린저 | 10,000,000,000 | 30,000,000,000 | 205.0% | 18% | -60% | 320% |
| 레전드 | 30,000,000,000 | 100,000,000,000 | 225.0% | 15% | -60% | 300% |
| 거장 | 100,000,000,000+ | — | 250.0% | 20% | -60% | 500% |

> **티어 단조성 보장**: 단조성은 `mu_tier` 단독으로 보장한다. `r_min`은 전 티어 동일(-60%)이며,
> 고티어도 손실 가능하다. 이는 "고티어가 항상 고수익"을 강제하던 구 설계의 부작용을 제거하기 위한
> 의도적 결정이다. 수익률 분포의 겹침(overlap)은 `sigma_tier`로 자연 발생한다 (AC-02).
> 또한, 모든 AI의 일별 누적 수익률은 `PriceEngine.DAILY_LIMIT_PCT`(±30%/일) 기반 선형 누적
...
target_r[42] = rng.randfn(25.0, 55.0)  → 가정값 38.6%
target_r[42] = clamp(38.6, -60.0, 600.0) = 38.6%
```
...
| EC-05 | `cumulative_r[d]`가 일별 PriceEngine 제한을 초과하는 경우 | `_generate_cumulative_returns()`에서 `PriceEngine.DAILY_LIMIT_PCT × (d+1)`을 상하한으로 클램프. 예: 1일차 ±30%, 2일차 ±60%, 20일차 ±600%. 단, `target_r`은 이미 `r_max`로 상한 제어되므로 클램프는 하한(폭락 경로) 보정이 주 목적 |
| EC-06 | 거장 AI(200명)의 `get_all_return_pcts` 요청 | 정상 반환. 거장 AI는 `mu_tier=250%`으로 높은 평균 수익률 분포를 가지며, 전 티어 동일하게 `r_min=-60%`. 글로벌 상위권을 점유하는 것은 의도된 설계이나, 손실 가능성도 존재한다 |
...
기대:
  - get_tier_return_pct(id) → float 반환, 범위 [-60%, r_max_tier]
