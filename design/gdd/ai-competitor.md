...
> **동일 티어 반복 시즌 난이도 설계 의도 (4-F)**: 플레이어가 승급하지 못하고 같은 티어를 반복할 때,
> AI 난이도는 **동일하게 유지**된다 (`mu_tier`, `sigma_tier`는 티어 고정값이며 시즌 반복 횟수와 무관).
> 설계 근거: 이 게임은 "숙달할수록 어려워지는" 점증 난이도 구조가 아니라 "자본 규모가 티어를 결정"하는 구조다.
> 같은 티어에 반복 진입하는 플레이어는 동일한 분포의 AI를 만나지만, 플레이어 스스로 스킬 트리를 해금하고
> 전략을 익힘으로써 실질 승률이 올라가는 구조다.
> 반복 시즌마다 `seed`가 달라지므로 AI 개별 수익률은 매번 재생성되어 패턴 학습 익스플로잇은 차단된다.
> 만약 특정 티어에서 3회 이상 연속으로 승급 실패 시 "동기부여 위기" 위험이 있으므로,
> 향후 "특별 시즌 이벤트" 또는 "난이도 조정 스위치" 추가를 고려할 수 있다 (scope: Sprint 12+ 후속 과제).
