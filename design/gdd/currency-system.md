...
- [ ] 게임 최초 시작 시 현금 자산이 정확히 1,000,000원으로 초기화됨
- [ ] 시즌 시작 전 현금 자산에서 티어 기준금액이 예수금으로 자동 입금됨 (잔여 현금 자산 유지)
- [ ] 매수 시 `sim_deduct()`로 예수금 정확히 차감, 매도 시 `sim_add()`로 정확히 증가
- [ ] 예수금이 음수가 되는 경우가 절대 없음
- [ ] 현금 자산이 음수가 되는 경우가 절대 없음
- [ ] 시즌 종료 시 보유 주식이 시장가로 전량 청산되어 예수금에 반영됨
- [ ] 시즌 종료 시 예수금 잔액 + 상금 전액이 현금 자산으로 입금됨. 예수금은 0으로 리셋됨
- [ ] 시즌 종료 시 순위에 맞는 상금이 현금 자산에 정확히 입금됨
- [ ] 라이프스타일 소비 시 `cash_deduct()`로 현금 자산에서 차감됨 (예수금 영향 없음)
- [ ] 포트폴리오 시스템이 계산하는 총 자산에 `get_cash_assets()` + `get_sim_cash()` 값이 정확히 반영됨
- [ ] 현금 자산 < 브론즈 기준금액 상태에서도 시즌 참가 가능 (전액 예수금 입금 → 프리마켓 티어)
- [ ] 성능: 잔액 조회/변경이 1ms 이내
...
- [ ] `CurrencySystem.get_cash_assets() -> int` — 신규 추가 필요
- [ ] `CurrencySystem.cash_add(amount: int)` — 신규 추가 필요
- [ ] `CurrencySystem.cash_deduct(amount: int) -> bool` — 신규 추가 필요
- [ ] `CurrencySystem.settle_to_cash(prize_amount: int)` — 신규 추가 필요 (시즌 종료: sim_cash + 상금 → cash_assets, sim_cash = 0)
- [ ] `CurrencySystem.auto_deposit_to_sim(amount: int)` — 신규 추가 필요 (시즌 시작 전: cash_assets → sim_cash)
- [ ] `CurrencySystem.get_total_prize_earned() -> int` — 신규 추가 (구 `get_deposit()` 대체)
- [ ] `CurrencySystem.prize_earned(amount: int, new_total: int)` 시그널 — 신규 (구 `deposit_changed` 대체)
