...
| A3 (T3) | 재무제표 (ROE/PER/PBR) | 기업 펀더멘털 분석, 분기 실적 이벤트 |
| A4 (T4) | 섹터 비교 분석 | 11개 섹터 수익률 순위표, 드릴다운 |
...
| P3 (T3) | 섹터 ETF 거래 | 섹터 단위 투자, 즉시 체결, 섹터 로테이션 수혜 (A4 선행) |
...
| 레이어 | 설명 | 표시 화면 |
| ---- | ---- | ---- |
| **현금 자산** | 유일한 실제 보유 현금. 예수금 재원 + 라이프스타일 소비 재원. 초기값 100만원 | F3 |
| **계좌 총 평가금액** | 예수금 + 거래 P&L. 투자 대회 경쟁 기준 | F1/F2 |
| **총 자산** | 현금 자산 + 계좌 총 평가금액 + 유형자산 평가액 | F3 |

**유형자산**: 부동산·사치품 등. 고정 감가 + 확률 변동 + 시장 이벤트 바이어스로 평가. 처분 시 현금 자산 전환 가능.
...
[시즌 시작 전]
현금 자산 기준 진입 가능한 티어의 기준 금액 예수금 자동 입금
(현금 자산 ≤ 100만원이면 자동 전액 입금)
→ 예수금 추가 조정 가능 (100만원 미만 출금 불가)

[시즌 시작]
예수금 기준 티어 자동 결정

[시즌 중]
예수금 + 거래 P&L = 계좌 총 평가금액 (예수금 추가 입금 불가)

[장 종료 후 - 매일]
자동 라이프스타일 정산 (현금 자산 +-)
→ 정산 리포트 표시 (일별/주별)
→ 휴장 시간: 능동 구매 가능 (현금 자산에서)

[시즌 정산]
시즌 정산 리포트 → 라이프스타일 정산 화면
예수금 전액 현금 자산 전환 → [시즌 시작 전]
```
...
청년 투자자가 정보를 수집하고, 차트를 분석하고, 시장 흐름을 읽어내면서 매 시즌마다 성장하는 쾌감.
쪽방에서 출발해 1,000억 '투자의 거장'으로 도달했을 때의 카타르시스.
반대로 모든 것을 잃고 한강에 서는 순간의 무게감 — 선택이 결과를 만든다는 실감.

## Detailed Design

→ 각 시스템의 상세 설계는 개별 GDD를 참조한다.
  - 가격 엔진: `design/gdd/price-engine.md`
  - 스킬 트리: `design/gdd/skill-tree.md`
  - 시즌 매니저: `design/gdd/season-manager.md`
  - 저장/로드: `design/gdd/save-load.md`
  - 전체 목록: `design/gdd/systems-index.md`

## Formulas

→ 개별 시스템 GDD의 §Formulas 참조. 이 문서는 시스템을 정의하지 않고 개념을 서술한다.

## Edge Cases

→ 개별 시스템 GDD의 §Edge Cases 참조.

## Dependencies

이 문서는 모든 시스템 GDD의 최상위 컨텍스트다. 각 시스템 GDD가 이 문서에 의존한다.

## Tuning Knobs

→ 개별 시스템 GDD의 §Tuning Knobs 참조. 주요 레버:
- 시즌 길이 (거래일 수): `season-manager.md`
- 스킬 해금 XP 임계값: `skill-tree.md`
- 가격 엔진 변동성 배율: `price-engine.md`

## Acceptance Criteria

- [ ] 모든 기재 시스템의 개별 GDD가 Approved 상태다
- [ ] 코어 루프 (시장 열기 → 매매 → 장 종료 → 정산)가 E2E 시나리오 테스트를 통과한다
- [ ] 엔딩 5종 이상이 구현되고 조건이 플레이어에게 명확히 인지된다

## Implementation Checklist

이 문서는 구현 직접 소유자가 아니다. 각 시스템 GDD의 Implementation Checklist를 확인한다.

- [x] 코어 시스템 GDD 전체 작성 (`systems-index.md` 등록 확인)
- [x] 스킬 트리 A3/A4/P3 항목 추가 (2026-04-21)
