...
스킬 트리 외에 **총 자산 평가액** (보유 현금 + 포트폴리오 평가액 + 실물 자산)을 F3 하단에 표시한다.
...
│  └─────────────────────────────────────────────────────┘  │
├─────────────────────────────────────────────────────────────┤
│  총 자산 평가액  ₩123,456,789                                │  ← 하단 자산 패널
│  보유 현금 ₩80,000,000  │  포트폴리오 평가액 ₩43,000,000  │  실물 자산 ₩456,789│
└─────────────────────────────────────────────────────────────┘
...
| 요소 | 내용 | 데이터 소스 |
|------|------|-------------|
| 총 자산 평가액 (대형) | `₩NNN,NNN,NNN` | `보유 현금 + 포트폴리오 평가액 + 실물 자산` (합산) |
| 보유 현금 | `₩NNN,NNN,NNN` | `0` 고정 (Beta Sprint 9: `CurrencySystem.get_real_cash()` — LifestyleManager 구현 시) |
| 포트폴리오 평가액 | `₩NNN,NNN,NNN` | `PortfolioManager.get_total_assets()` (예수금 + 예약금 + 보유종목 평가액) |
| 실물 자산 | `₩NNN,NNN,NNN` | `0` 고정 (Beta Sprint 9: `LifestyleManager.get_tangible_value()` — LifestyleManager 구현 시) |
...
- [-] `LifestyleManager.get_tangible_value()` — Beta Sprint 9 (B-02) 이월. 그 이전 0 고정.
