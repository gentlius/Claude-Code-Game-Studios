...
자산·포트폴리오·거래내역 등 재무 정보는 F3에 포함하지 않는다 (F1 담당).
리그 순위는 포함하지 않는다 (F2 담당).
...
| `LifestyleManager` | → GrowthScreen | `get_current_residence()` — 현재 거주지 배경 이미지 경로. `get_unlocked_items()` — 사치품 레이어 오브젝트 목록. `get_earned_titles()` — 획득 칭호 목록. (Beta 구현 시 추가) |
