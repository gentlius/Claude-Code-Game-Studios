...
**두 종류의 라이프스타일 이벤트:**
- **자동 정산** (매 시즌 종료 후): 부동산 임대 수익, 스타트업 엑싯, Recurring 비용이 cash_assets에 자동 반영
- **능동 구매** (비시즌 소비 화면): 거주지 업그레이드, 사치품, 대안 투자, 사회공헌

---

## 2. Player Fantasy

시즌이 끝날 때마다 "이 돈으로 뭘 살까?"가 진짜 선택이 된다.
...
```
[시즌 종료 — 강제 청산 + 상금 정산 완료]
  ↓
[비시즌 자동 정산 — LifestyleManager.process_offseason()]
  ├─ 부동산 임대 수익 → cash_assets 입금
  ├─ 스타트업 엑싯 이벤트 (만기 도달 시) → cash_assets 입금 또는 0원
  └─ Recurring 비용 차감 (골프 클럽 연회비, 투자 클럽 연회비 등)
  ↓
[라이프스타일 소비 화면 — 능동 선택]
  ├─ 거주지 업그레이드
  ├─ 사치품 구매
  ...
  └─ 사회공헌
  ↓
[다음 시즌 시작 버튼] — 소비 후 잔여 cash_assets = 티어 배정 기준
```
...
| 품목 | 비용 | 해금 조건 | 보상 |
|------|------|---------|------|
| 장학재단 설립 | ₩500,000,000 일회성 | 마스터+ | F2 리그 프로필 재단명 표시, "사회공헌인" 칭호, XP +50 (시즌 1회 캡) |
...
| 부동산 종류 | 매입가 | 시즌 임대 수익 | 수익률 | 해금 조건 |
|-----------|--------|-------------|------|---------|
| 소형 오피스텔 | ₩200,000,000 | ₩5,000,000 | 2.5%/시즌 | 에메랄드+ |
| 강남 상가 | ₩1,000,000,000 | ₩30,000,000 | 3.0%/시즌 | 마스터+ |
| 빌딩 | ₩5,000,000,000 | ₩200,000,000 | 4.0%/시즌 | 그랜드마스터+ |
...
- 라이프스타일 소비 화면 진입 버튼: F3 우상단 (비시즌 기간에만 활성)
...
```
exit_multiplier:
  IPO (randf() < 0.20):        randf_range(2.0, 5.0)
  M&A (randf() < 0.50/0.80):   randf_range(1.0, 1.5)
  폐업 (그 외):                0.0
...
| 시스템 | 방향 | 내용 |
|--------|------|------|
| `SeasonManager` | Hard ← | 비시즌 윈도우 진입/종료 신호. `process_offseason()` 호출 시점 |
...
| `STARTUP_MA_PROB` | 0.50 | 0.30~0.60 | M&A 성사 확률 (IPO 실패 중) |
