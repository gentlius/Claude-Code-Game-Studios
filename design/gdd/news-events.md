...
> **VI/CB 시스템 이벤트 본문**: `is_system_event = true`인 이벤트(VI 발동/해제, CB 1·2단계)도
> `body` 필드를 포함한다. 본문은 `news_event_system.gd` 내에서 코드로 직접 조립되며
...
| 스킬 레벨 | player_delay_ticks | 실시간 환산 (1x) | 캔들(M1) | 설명 |
|----------|-------------------|----------------|---------|------|
| T0 (기본) | 20틱 (5분×4TPM) | 약 3.8초 (20×0.192) | 5캔들 | 가격 움직임 보고 이유 모르는 구간. 10캔들 이상이면 인과관계가 단절되어 보여 UX 저하 |
| T1 (S1 해금) | 8틱 (2분×4TPM) | 약 1.5초 (8×0.192) | 2캔들 | 반응 시간 2.5배 향상. T2("즉시") 가치 보존 위해 0이 아닌 8틱 유지 |
| T2 (S2 해금) | 0틱 | 즉시 | 0캔들 | 이벤트와 동시에 뉴스 표시 — 스킬 트리 핵심 마일스톤 |
| T3 (S3 해금) | -60틱 (선행) | 약 11.5초 전 (60×0.192) | 15캔들 선행 | 루머 채널 (5-4 참조). `rumor_advance_ticks`와 동일 값. (선행, 개념적 표현 — 실제 구현 시 rumor_advance_ticks(양수 60)로 처리. 규칙 5-4 참조) |
...
방향(호재/악재)을 포함하되, 정확도 55% — 45% 확률로 방향이 반전된 잘못된 힌트. (B-09: 70%→55% 하향. RUMOR_BASE_ACCURACY = 0.55)
...
player_delay_ticks: T0=20 (5min×4TPM), T1=8 (2min×4TPM), T2=0
```
...
| `NEWS_DELAY_T0` | 5분 (=20틱) | 3~10분 | @export | T0 뉴스 딜레이 (분 단위, ×TPM으로 틱 변환). 10분(40틱)은 M1 10캔들 지연으로 UX 단절감 유발 |
| `NEWS_DELAY_T1_MIN` | 2분 (=8틱) | 1~5분 | @export | T1 뉴스 딜레이 (S1 해금 시). T0=20틱에서 8틱으로 단축, T2(0틱) 가치 보존 |
...
| 가격 엔진 | 가격 엔진이 이벤트를 소비 | Event 오브젝트 전달. **Hard** (가격 엔진 입장). 역방향 참조: `price-engine.md` §Dependencies 참조 |
| 트레이딩 스크린 | 트레이딩 스크린이 뉴스 이벤트를 소비 | 뉴스 피드 UI를 통해 카드 표시. 뉴스 딜레이/감속 기능(`S2` 스킬 연계). 역방향 참조: `trading-screen.md` §Dependencies 참조 |
...
- [ ] T0에서 20틱(5분×4TPM) 딜레이 후 뉴스가 표시됨
