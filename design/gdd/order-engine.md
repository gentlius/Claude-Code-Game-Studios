...
| CB Stage 1 정지 중 PRE_MARKET 큐 및 지정가 처리 | CB 발동 시 Order Engine은 ACCEPTING 상태를 유지하나, 가격 엔진이 전 종목 가격을 동결하므로 시장가/지정가 모두 **체결 유예**. 정지 해제 후 첫 틱에 가격 갱신 → 큐 순서대로 처리 재개. 단, CB가 틱 1에 발동되는 경우(극히 드문 케이스): pre_market_queue는 가격 엔진 갱신 후 처리되므로, 갱신된 가격으로 체결 시도 → CB 발동으로 가격 동결 → 다음 틱으로 이월 | CB는 가격 엔진 레벨에서 동결. 주문 엔진은 간접적으로 유예됨 |
