...
플레이어의 매매는 두 가지 경로로 가격에 영향을 준다: (1) ADR-019 `_player_pressure` — 체결량이 tick_volumes에 반영되어 다음 틱 delta에 누적, (2) `order-book.md` (Approved) — 플레이어 주문이 호가 잔량을 실제로 소진하며 슬리피지와 가격 이동 발생.
...
| MacroState | 값 | 설명 |
|------------|----|------|
| `TREND_UP`   | 0 | 상승 추세일. 다음 날도 상승 추세 유지 확률 96% |
| `FLAT`       | 1 | 보합일 (초기값). 유지 확률 96% |
| `TREND_DOWN` | 2 | 하락 추세일. 유지 확률 96% |

평균 지속 일수: UP ≈ 25일, FLAT ≈ 25일, DOWN ≈ 25일. 자기유지 확률 0.96 → 1시즌(20일)을
단일 MacroState가 지배할 확률 ≈ 80%. 월봉 차트에서 뚜렷한 방향성이 형성된다.

> **ADR-026 v2 (2026-04-23)**: 기존 self-prob 0.92~0.93(12.5~14.3일)에서 0.96(25일)으로 증가.
> 월봉 박스 패턴 해결을 위한 개선. driftScale 동시 도입.

###### 매크로 바이어스 메커니즘

매 거래일 시작 시(C++ day 루프 최상단, GDScript `_end_trading_day()` 호출 직전):

1. **SeasonBias 매크로 넛지**: 시즌 바이어스에 따라 FLAT 행의 전환 확률 조정:
   - `BULL`: FLAT→TREND_UP +0.06, FLAT→TREND_DOWN −0.02 (BULL 시즌에 상승 추세 달이 더 자주 발생)
   - `BEAR`: FLAT→TREND_DOWN +0.06, FLAT→TREND_UP −0.02 (역방향)
   - `NEUTRAL`: 조정 없음
   - 행 합이 1이 되도록 재정규화
2. `macro_tm[current_macro_state]` 행 기준으로 다음 MacroState 전환 롤
3. 선택된 MacroState에 따라 미시 전환 행렬을 컬럼 바이어스 적용:
   - `TREND_UP`: STRONG_UP(0)·UPTREND(1) 컬럼 × `biasFactor`, 행 재정규화
   - `FLAT`: 행렬 무변경 (no-op)
   - `TREND_DOWN`: DOWNTREND(3)·STRONG_DOWN(4) 컬럼 × `biasFactor`, 행 재정규화
4. **driftScale 적용**: TREND_UP/DOWN 기간 중 드리프트 레이어의 `k_drift`를 `driftScale[state]`배 감소
   - 기본값: TREND_UP/DOWN → 0.2× (80% 감소), FLAT → 1.0× (무변경)
   - 효과: 평균회귀 평형점이 0.875%에서 ~11%로 확장 → 월봉에 실제 가격 변동 가시화
5. 해당 일의 M1 거래량에 `macro_vol_mult` 곱셈 적용

결과: TREND_UP 기간에는 미시 상태가 UPTREND로 더 자주 전환하고, 드리프트 억제 효과로 가격이
실제로 상방향으로 이동한다. 20일(1시즌) 집계 시 월봉에서 명확한 상승/하락 추세가 형성된다.

> **SeasonBias 역할 분리**: 기존 7×7 미시 행렬에 대한 SeasonBias 넛지(BIAS_UP additive)는
> 평형점 최대 0.375%로 월봉에 무효였음이 수학적으로 확인됨. ADR-026 v2에서 SeasonBias는
> 3×3 매크로 전환 행렬에만 적용 → "어떤 달이 추세 달인가"를 결정. "추세 강도"는 MacroState가 담당.

###### 아키타입별 매크로 전환 행렬

| Archetype | 특성 | UP 평균 지속 | DOWN 평균 지속 |
|-----------|------|------------|--------------|
| `GROWTH` | UP ultra-sticky | 33.3일 | 10.0일 |
| `VALUE_DIVIDEND` | FLAT ultra-sticky (33일) | 8.3일 | 8.3일 |
| `CYCLICAL` | 빠른 전환 | 10.0일 | 10.0일 |
| `EVENT_DRIVEN` | FLAT 33일 → 이벤트 폭발 | 6.7일 | 6.7일 |
| `RECOVERY_UNCERTAIN` | DOWN 편향 (20일) | 8.3일 | 20.0일 |
| `DECLINING_TRAP` | DOWN ultra-sticky (33일) | 6.7일 | 33.3일 |
...
`assets/data/price_engine_config.json` → `macroTrend` 키 (config version 5):
- `transitionMatrix`: 3×3 기본 전환 행렬 (self-prob 0.96, avg 25일)
- `biasFactor`: 컬럼 바이어스 배수 (기본 3.0; 안전 범위 [1.5, 5.0])
- `volMultiplier[3][2]`: MacroState별 일별 거래량 배수 범위
- `driftScale[3]`: MacroState별 k_drift 배율 ([TREND_UP, FLAT, TREND_DOWN] = [0.2, 1.0, 0.2])
- `seasonBiasMacroNudge`: BULL/BEAR 시즌이 FLAT 행에 가하는 additive 넛지
...
| `macro_tm 자기유지` | 0.96 | 0.88~0.97 | 추세 지속 기간 증가 (1/(1-p)일). 0.96=25일, 아키타입별 상이 | 추세 빠르게 역전. 잦은 방향 전환 |
| `driftScale (TREND_UP/DOWN)` | 0.2 | [0.05, 0.5] | 낮을수록 추세 기간 중 가격 이동 폭 증가. 0.2→평형 ~11% | 1.0 → 0.875% 평형, 월봉 박스 패턴 |
| `seasonBiasMacroNudge (BULL flatToUp)` | +0.06 | [0.0, 0.15] | BULL 시즌에 상승 추세 달 발생 빈도 증가 | 0 → 시즌 바이어스 매크로 레이어 무효 |
...
- [x] **ADR-026 v2**: `price_engine_config.json` v5 — `driftScale: [0.2, 1.0, 0.2]` + `seasonBiasMacroNudge` + self-prob 0.96 (기본+6아키타입 모두 갱신)
- [x] **ADR-026 v2**: C++ `_macro_drift_scale[3]` 멤버 + `set_config()` driftScale 파싱 + drift 공식에 `_macro_drift_scale[macro_state]` 적용
- [x] **ADR-026 v2**: GDScript `_compute_drift_delta(macro_state)` — `driftScale` 적용
- [x] **ADR-026 v2**: GDScript `_roll_macro_states()` — SeasonBias 넛지를 3×3 macro_tm FLAT 행에 적용 (기존 7×7 미시 행렬 넛지 대체)
- [x] **ADR-026 v2**: `CACHE_VERSION 5→6` bump (알고리즘 변경으로 pre-history 캐시 무효화)
