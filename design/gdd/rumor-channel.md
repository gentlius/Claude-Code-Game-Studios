...
**70% 정확 루머**: 선취매 → 60틱 동안 가격이 올라준다 → 뉴스 발동 순간 매도 → 이중 수익.
**30% 가짜 루머**: 선취매 → 가격이 올라간다 → 뉴스 발동 순간 악재 → 급락. 설거지당한다.

정확도 70%를 체득한 플레이어만 루머와 차트를 교차 검증해 전략을 세울 수 있다.
루머는 더 이상 알림이 아니라 **고위험 선행 트레이딩의 출발점**이다.
