...
| `ShortSellingSystem` *(TR3 구현 시)* | short_positions (Array[Dictionary]: stock_id, quantity, entry_price, margin_deposited, open_day) |
| `LeverageManager` *(TR4 구현 시)* | leverage_positions (Array[Dictionary]: stock_id, quantity, entry_price, multiplier, borrowed, accrued_interest, open_day) |
| `EtfManager` *(P3 구현 시)* | etf_holdings (Array[Dictionary]: etf_id, quantity, avg_buy_price, total_invested), sector_flow (Dictionary: sector→float) |
| `LifestyleManager` | residence_tier (int), owned_properties (Array[{property_id, purchase_price}]), startup_investments (Array[{amount, grade, expiry_season}]), owned_items (Array[String: item_id]), titles (Array[String]) |
...
```json
{
  ...
  "short_selling": {
    "short_positions": [
      { "stock_id": "005930", "quantity": 5, "entry_price": 78000, "margin_deposited": 195000, "open_day": 3 }
    ]
  },
  "leverage": {
    "leverage_positions": [
      { "stock_id": "ST001", "quantity": 100, "entry_price": 65000, "multiplier": 2, "borrowed": 3250000, "accrued_interest": 2600, "open_day": 7 }
    ]
  }
}

> **short_selling / leverage 섹션**: TR3/TR4 구현 시 포함됨. `save_version 3`에서 해당 키가 없는 세이브는
> 빈 배열(`[]`)로 기본값 처리한다. 필드 의미는 `leverage-trading.md §3-1` 및 `short-selling.md §3` 참조.
```
...
| AC-18 (E2E) | 레버리지 포지션 보유 중 장 마감 저장 → 재시작 → 로드 시 `leverage_positions`(`borrowed`, `accrued_interest` 포함) 정확히 복원되어 마진콜 체크 즉시 재개 가능 (TR4 구현 시 검증) |
| AC-19 (E2E) | 공매도 포지션 보유 중 장 마감 저장 → 재시작 → 로드 시 `short_positions`(`entry_price`, `margin_deposited` 포함) 정확히 복원 (TR3 구현 시 검증) |
