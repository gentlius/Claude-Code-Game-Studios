...
> **UX 노트**: 공매도 포지션 모니터링(margin_ratio, unrealized_pnl, 대차 수수료 누적)은 주문 패널 단일 뷰에서 롱 포지션과 통합 표시 예정 (Sprint 11 UX 범위).
...
**예시 (가격 상승 후 손실)**: unrealized_pnl=-300,000 (current_price=205,000, qty=10)
- `margin_ratio = (2,450,000 + (-300,000)) / 1,750,000 = 2,150,000 / 1,750,000 = 1.229`

**예시 (임계값 근접 — 강제청산 발동 직전, 4-C)**: current_price=385,000 (개시가 175,000 대비 +120%), qty=10
- `unrealized_pnl = (175,000 - 385,000) × 10 = -2,100,000`
- `margin_ratio = (2,450,000 + (-2,100,000)) / 1,750,000 = 350,000 / 1,750,000 = 0.200`
- → **강제청산 발동** (0.200 ≤ 0.20). `remaining = 350,000원` (초기 투자 1,750,000 중 80% 손실)
- → 실제 순손실: `350,000 - 385,000×10 = 350,000 - 3,850,000`. `sim_add(350,000)`, `sim_deduct(3,850,000)` → 순 -3,500,000원
...
### F7. 대차 수수료 (Borrow Fee)

```
# 매 거래일 장 마감 시 처리 (on_market_close)
for each open short_position:
    daily_fee = open_price × quantity × BORROW_FEE_RATE_DAILY
    CurrencySystem.sim_deduct(daily_fee)   # 예수금에서 차감
    short_position.accrued_fee += daily_fee
```

| Variable | Type | Range | Source | Description |
|----------|------|-------|--------|-------------|
| `BORROW_FEE_RATE_DAILY` | float | 0.0001~0.001 | market_config.json | 일별 대차 수수료율. KR 기본값 0.0003 (0.03%/일 ≈ 연 6%) |
| `open_price` | int | 1+ | ShortPosition | 숏 진입가 |
| `quantity` | int | 1+ | ShortPosition | 보유 주수 |
| `accrued_fee` | float | 0+ | ShortPosition | 누적 대차 수수료 (UI 표시용) |

**예시**: open_price=50,000, quantity=100주, 10일 보유
- 일별 대차 수수료 = 50,000 × 100 × 0.0003 = **₩1,500/일**
- 10일 누적 = ₩15,000 (포지션 가치 ₩5,000,000의 0.3%)
...
|-----------|--------------|------------|----------|-------------------|-------------------|
| `BORROW_FEE_RATE_DAILY` | 0.0003 (0.03%/일, ≈연 6%) | 0.0001 ~ 0.001 | Curve | 장기 보유 페널티 심화. 단기 트레이딩 유도 | 장기 숏 전략 수익성 향상. 보유 기간 패널티 약화 |
...
>
> **대차 수수료(Borrow Fee)**: KRX 현실 규칙에 따라 보유 기간 비례 대차 수수료를 적용한다.
> 매 거래일 장 마감(`on_market_close`) 시 열린 숏 포지션 전체에 대해 자동 차감.
> 공식: §F7 참조. 장기 숏 포지션은 수수료가 누적되어 자연스러운 보유 기간 패널티가 발생한다 — 의도된 전략 구조.
...
| AC-22 | 매 장 마감 시 열린 숏 포지션에 `BORROW_FEE_RATE_DAILY × open_price × quantity` 대차 수수료 차감 | 단위 테스트: 숏 포지션 3일 유지 후 sim_cash 감소분 = daily_fee × 3 |
