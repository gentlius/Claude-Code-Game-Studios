...
4개 브랜치(분석 도구, 시장 감지, 거래 스킬, 포트폴리오)로 구성되며, 각 브랜치는
4~5단계의 순차 해금 구조를 가진다. 스킬 해금은 영구적이며 시즌 리셋에 영향받지
...
MVP(Lv2 = 스킬 포인트 1개)에서 접근 가능한 T1 스킬:
...
rumor_lead_time = RUMOR_LEAD_TICKS    # 뉴스 발생 60틱(~45초) 전
```
...
| NEWS_DELAY_T0 | 40틱 (~30초) | 20~60 | 초기 정보 불이익 크기 | 너무 크면 뉴스 시스템 무용 |
| NEWS_DELAY_T1 | 20틱 (~15초) | 10~30 | S1 해금 가치 | T0과 차이 적으면 해금 동기 부족 |
| RUMOR_BASE_ACCURACY | 70% | 50~90% | 루머 채널 가치 | 90%+면 확실한 선행 정보 = 밸런스 파괴 |
| RUMOR_LEAD_TICKS | 60틱 (~45초) | 30~120 | 루머 선행 시간 | 120+면 과도한 이점 |
...
- 공매도/레버리지는 MVP 범위 밖 (주문 엔진 GDD에서 Lv4-5로 분류). V-Slice에서 스킬 트리에 표시하되 해금 불가로 처리할지, 아예 숨길지 결정 필요
