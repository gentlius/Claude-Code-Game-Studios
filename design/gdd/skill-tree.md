...
| 모든 스킬 해금 후 추가 스킬 포인트 | 잉여 포인트 누적. 향후 확장 콘텐츠용 (Sprint 12+ 추가 스킬 예정) |

> **스킬 소진 구간 — 의도된 설계**: 모든 스킬을 해금한 플레이어는 이후 XP/스킬 포인트가 더 이상 즉각적 이득을 주지 않는다. 이 구간은 "보조 도구 없이 순수 판단 실력으로 경쟁하는 구간"으로 **의도적으로 설계**됐다 — 필라 "판단이 곧 실력" 직접 구현. 스킬 포인트 잉여 활용처는 Sprint 12+에서 추가 스킬 브랜치(고급 분석 도구)로 구현 예정.
