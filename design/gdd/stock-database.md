...
| `archetype` | string | 마르코프 전환 행렬 선택 키. 빈 문자열이면 기본 대칭 행렬 사용. (ADR-025) | "GROWTH" |
| `season_drift` | float | 시즌당 base_price 누적 이동률. 양수=장기 상승, 음수=장기 하락. (ADR-025) | 0.025 |
...
### F1. 시즌별 기준가 복리 이동 (ADR-025, 구현 완료)

시즌 2부터 매 시즌 경계에 `season_drift`를 복리로 base_price에 적용한다.
첫 시즌은 JSON 원본값을 그대로 사용. 평균 회귀 앵커와 하드 클램프 창이 함께 이동한다.

```
new_base_price = round_to_tick(prev_base_price × (1 + season_drift))
new_base_price = max(new_base_price, 1000)   # 최소 1,000원 클램핑
```

| Variable | Type | Range | Source | Description |
|----------|------|-------|--------|-------------|
| prev_base_price | int | 1,000-1,000,000 | PriceEngine._stock_states | 직전 시즌 적용 후 기준가 |
| season_drift | float | -0.05 ~ +0.05 | StockData.season_drift | 종목별 시즌당 이동률. JSON에서 로드 |
| new_base_price | int | ≥ 1,000 | PriceEngine._stock_states | 이번 시즌 평균 회귀 앵커 |

**예시** (GROWTH 종목, season_drift = +0.025, 초기 base_price = 100,000원):
...
**저장**: `base_price`는 PriceEngine 세이브 데이터에 종목별로 저장됨.
구버전 세이브 호환: 저장값 없으면 StockData.base_price로 폴백.

> **season_theme_modifier (이전 설계)**: 시즌 테마 계수 방식은 폐기.
> season_drift 복리 방식으로 대체됨 (ADR-025).
...
| `season_drift` (per stock) | -0.015 ~ +0.035 | -0.05 ~ +0.05 | 시즌 간 base_price 이동 가속 | 이동 감속. 0.0이면 시즌 간 기준가 불변 |
| archetype 행렬 비대칭도 | 50% (Phase 1) | 0~100% | 종목 간 방향성 차이 뚜렷해짐 — 박스 전략 무력화 | 0%=대칭 행렬(구버전), 전략 차별화 사라짐 |
...
- [x] `stocks.json` → `stocks_kr.json`으로 이름 변경 (원본 stocks.json은 마스터 사본으로 유지)
- [x] `StockDatabase` — `STOCK_DATA_PATH_TEMPLATE % market_id` 동적 경로로 로드 (`set_active_market()`)
- [ ] `stocks_us.json`, `stocks_jp.json` 스텁 파일 생성 (빈 배열 `[]`) — DLC 번들 슬롯 확보
- [x] 섹터 목록도 `market_kr.json`에 `"sectors": ["반도체", "2차전지", ...]`로 등록 — 하드코딩된 섹터 상수 제거
- [x] 기존 `stocks.json` 참조 전체 검색 후 `stocks_kr.json`으로 교체 완료
