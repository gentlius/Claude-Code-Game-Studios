...
두 가지 자동 매도 조건을 사전 설정해두면, 매 틱마다 현재가를 감시하다가 조건이 충족되는 순간
OrderEngine을 통해 시장가 매도를 자동 발동하는 TR2 스킬 기능이다. TR1 지정가 주문이
"내가 원하는 가격에 사는" 도구라면, TR2는 "내가 설정한 리스크 한계 밖으로 가격이 이탈하면
자동으로 판다"는 감시·보호 도구다. 손절/익절 설정은 종목별로 독립적이며, 하나의 종목에
손절가와 익절가를 동시에 설정할 수 있다. 발동 후 자동으로 해당 종목의 설정이 해제된다.
...
발동 시 `_submit_auto_market_sell()` 내부에서:

1. `OrderEngine.submit_market_order("SELL", stock_id, qty)` 호출
2. 반환 주문 dict에 `is_auto_trigger: true`, `trigger_reason: "STOP_LOSS" | "TAKE_PROFIT"` 메타 추가
3. `on_stop_take_triggered(stock_id, trigger_reason, filled_price)` 시그널 발행 → UI 알림
...
| 이벤트 | 처리 |
|--------|------|
| TR2 해금 | 기존 보유 종목에 설정 가능. 초기 설정 없음 |
| 종목 전량 수동 매도 | 해당 종목 설정 자동 삭제 |
| 발동 후 전량 체결 | 설정 자동 삭제 |
...
STOP_LOSS 발동:
    ...
    AND market_state == MARKET_OPEN
```

- `stop_loss_price`: 플레이어 설정 손절 기준가 (원, 호가 단위 배수)
- `current_price`: PriceEngine.get_current_price(stock_id) 반환값

예시: 보유가 65,000원, 손절가 62,000원. 현재가 61,500원 → **발동**.
...
예시: 보유가 65,000원, 익절가 72,000원. 현재가 72,500원 → **발동**.

### F3. 자동 매도 수량 결정

```
auto_sell_qty = min(setting.quantity, holding.available_quantity)
```

- `available_quantity`: 현재 가용 수량 (지정가 매도 잠금 수량 제외)
...
> **GDD 동기화 필요**:
> - `design/gdd/order-engine.md` — 틱 처리 순서 3-d 단계 추가
> - `design/gdd/skill-tree.md` — Dependencies 테이블에 StopTakeSystem 추가
