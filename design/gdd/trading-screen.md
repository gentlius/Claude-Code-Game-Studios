...
> - 일반 평일: `["daily"]` → 일일 정산 확인 → **소비 화면** → 다음 날
> - 금요일: `["daily", "weekly"]` → 일일 → 주간 리포트 순차 확인 → **소비 화면** → 다음 주
> - 시즌 마지막 금요일: `["daily", "weekly", "season"]` → 일일 → 주간 → 시즌 결과 확인 → **(시즌 자동 정산)** → **소비 화면** → 다음 시즌
>
> 각 리포트 사이 "다음 → Enter" 버튼으로 확인 후 다음 리포트 표시. 마지막 리포트 확인 후 `GameMain._show_lifestyle_screen()` 호출. 소비 화면에서 "다음 날/시즌" 버튼 클릭 시 `GameClock.confirm_transition()` 호출.
>
> **소비 화면 버튼 텍스트**: 시즌 마지막 금요일 = "다음 시즌 시작 →", 그 외 = "다음 날 →"
>
> **시즌 자동 정산 타이밍**: `LifestyleManager.process_offseason()`은 시즌 결과 리포트 확인 직후, 소비 화면 표시 직전에 실행. (`design/gdd/lifestyle-spending.md §3-1` 참조)
...
| **SettlementReporter** | `GameClock.on_market_close`, `on_week_end`, `on_season_end` | `settlement_confirmed()` | `GameMain._show_lifestyle_screen()` 호출 → 소비 화면 "다음 날/시즌" 클릭 → `GameClock.confirm_transition()` 호출 |
...
| 정산 확인 | `SettlementReporter.settlement_confirmed` → `TradingScreen` → `GameMain._show_lifestyle_screen()` → 소비 화면 닫기 → `GameClock.confirm_transition()` |
