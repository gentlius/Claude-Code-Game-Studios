...
| 현금 잔액 | 시드: ₩300,000 (예약: ₩200,000) | 매매/주문 제출·체결·만료 시 |
...
**예약금 표시 규칙**: `reserved_cash`는 주문 엔진의 `get_total_reserved_cash()`로
조회한다. 미체결 지정가 매수 주문이 없을 때(= 0) 예약 표시를 숨긴다. 예약금이
