...
| B-11 | 오더북 구현 (`order-book.md` In Review, 코드 없음) — 호가창 + 슬리피지 + 가격 영향 모델 전환 | Sprint 8 | 📋 Planned |
| B-12 | 라이프스타일 소비 시스템 (`lifestyle-spending.md` Draft) — GDD 완성 + LifestyleManager 구현 + 오프시즌 화면 | Sprint 9 | 📋 Planned (B-02 주거 아트 선행 조건) |
...
| 8 | 2026-04-29 ~ 2026-05-12 | A3 + S3 루머 + 오더북 | B-06(A3 재무제표) + B-07a(S3 루머) + B-09(설정화면) + B-11(오더북) |
| 9 | 2026-05-13 ~ 2026-05-26 | TR3 + TR4 + 주거시각화 + 라이프스타일 | B-07b(TR3 공매도) + B-07c(TR4 레버리지) + B-02(주거시각화, Should Have, 아트 조건부) + B-12(라이프스타일, B-02 선행 조건부) |
...
> - B-12 `lifestyle-spending.md` (Draft skeleton) → Sprint 9 (B-02 주거 아트 선행 조건부)
