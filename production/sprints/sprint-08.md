...
분석 스킬 브랜치를 확장한다. 설정 화면으로 접근성·오디오 컨트롤을 마무리한다.
...
| S8-06 | 설정 화면 GDD + 구현 (B-09) | game-designer + ui-programmer | 1.5 | — | **(1)** `design/gdd/settings-screen.md` 9개 섹션 (볼륨·접근성·키 리맵·뉴스 자동 감속 토글). **(2)** F4 또는 메뉴에서 설정 화면 접근. **(3)** 볼륨 슬라이더 → `AudioManager` 즉시 반영 + 세이브 파일 유지. **(4)** 뉴스 자동 감속 On/Off 토글 → `NewsEventSystem` 반영. **(5)** 빌드 성공. |

### Should Have

| ID | Task | Agent/Owner | Est. Sessions | Dependencies | Acceptance Criteria |
|----|------|-------------|---------------|-------------|---------------------|
| S8-07 | Beta QA 10일 시나리오 — A3·S3 검증 추가 | qa-lead | 0.5 | S8-03, S8-05 완료 | A3 재무제표 표시 확인 + S3 루머 발생 확인을 `docs/testing/` QA 시나리오에 추가. 기존 10일 자동화 시나리오에 S3 루머 수신 assert 포함. |
| S8-08 | 오더북 ADR 작성 (ADR-019) | technical-director | 0.5 | S8-01 완료 | `docs/architecture/019-order-book-price-impact-model.md` 작성: 호가창 슬리피지 공식 선택 근거, 장 마감 폐기 결정 이유. `technical-preferences.md` ADR 목록 추가. |

### Nice to Have

| ID | Task | Agent/Owner | Est. Sessions | Dependencies | Acceptance Criteria |
|----|------|-------------|---------------|-------------|---------------------|
| S8-09 | start-screen.md Implementation Checklist 검증 | qa-lead | 0.5 | — | `design/gdd/start-screen.md` Checklist 항목 실제 구현 여부 확인. 미구현 항목 tech-debt 등록 또는 `[x]` 체크 완결. |
...
| Must Have 합계 | 7.0 (S8-01×2.5 + S8-02×0.5 + S8-03×1 + S8-04×0.5 + S8-05×1 + S8-06×1.5) |
| Should Have | 1.0 |
| Nice to Have | 0.5 |
| **Must Have 적합성** | **7.0 / 8 ✅** |
| **전체 합계** | **8.5 (buffer 1.5 세션 여유)** |
...
- [ ] S8-06: 설정 화면 접근 + 볼륨 저장 E2E + 뉴스 감속 토글 확인 + 빌드 성공
- [ ] S8-07: QA 시나리오 문서 업데이트
- [ ] S8-08: ADR-019 작성 + `technical-preferences.md` 링크 추가
