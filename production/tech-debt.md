...
- ~~뉴스 날짜 월 하드코딩 (M, 3월 고정)~~ — **해결됨 (2026-04-05)**: 픽션 날짜 체계 도입. `SeasonManager.get_fiction_date()` — 시즌 순번에 따라 Q1(1월)/Q2(4월)/Q3(7월)/Q4(10월) 자동 순환. `news_event_system.gd` `{date}` 치환이 SeasonManager를 통해 동적 월을 반영.
